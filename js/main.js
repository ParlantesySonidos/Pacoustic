...
const SOCIAL_FACEBOOK = "https://www.facebook.com/share/18PTtov6m2/";

const SOCIAL_INSTAGRAM = "https://www.instagram.com/parlantesysonido?igsh=MTh2MXNxNDdveDd0bQ==";
...
function introAudioInHeroZone() {
    ...
    return bottom > window.innerHeight * .52;
}
...
function initIntroAudio() {
    ...
    function stop() {
        let fade = setInterval(() => {
            if (audio.volume > .05) audio.volume -= .05; else {
                clearInterval(fade);
                audio.pause();
                audio.volume = .6;
                introAudioState.started = false;
...
function refreshCatalogViews() {
    try {
        document.dispatchEvent(new CustomEvent("pacoustic:catalog-ready", {
            bubbles: true
        }));
    } catch (_) {}
    renderBanner();
...
async function loadProducts() {
    try {
        ...
        products = data.map(p => {
            ...
            const nid = typeof p.id === "number" && !Number.isNaN(p.id) ? p.id : parseInt(String(p.id), 10) || 999999;
            return {
...
function renderBanner() {
    ...
    if (!track) return;
    const items = products.filter(p => p.bannerImg && p.bannerImg.trim() !== "").map(p => ({
        src: p.bannerImg,
        alt: p.name,
        id: p.id
    }));
...
function relayoutAllBannerItems() {
    ...
    if (!track) return;
    const vw = window.innerWidth || document.documentElement.clientWidth || 400;
    const vh = Math.max(window.innerHeight || 0, window.visualViewport?.height || 0, document.documentElement?.clientHeight || 0) || 600;
    ...
    const rowCap = vw <= 768 ? Math.min(vh * .82, 780) : Math.min(vh * .94, 920);
    const maxH = getBannerMaxDisplayHeightForWidth(track, w);
    let H;
    if (maxH < 1) {
        if (vw <= 768) {
            H = Math.round(Math.min(Math.max(vw * .72, 280), rowCap));
        } else {
    ...
    const wrap = track.parentElement?.classList?.contains("banner-wrap") ? track.parentElement : document.querySelector(".banner-wrap");
    if (wrap) {
        if (vw <= 768) wrap.style.setProperty("--banner-slide-w", `${Math.round(w)}px`); else wrap.style.removeProperty("--banner-slide-w");
    }
...
function bindBannerItemImage(img) {
    ...
    if (img.complete && img.naturalWidth > 1) run(); else {
...
function ensureBannerLayoutObserver() {
    ...
    _bannerLayoutIo = new IntersectionObserver(() => scheduleBannerRelayout(), {
        root: null,
...
function getVideoEmbed(url) {
    ...
    if (url.includes("res.cloudinary.com") && url.match(/\.(mp4|webm|mov)(\?|$)/i)) {
        const thumb = url.replace("/video/upload/", "/video/upload/so_0/").replace(/\.(mp4|webm|mov)(\?|$)/i, ".jpg");
        return {
            type: "video",
            src: url,
            thumb: thumb
    ...
    if (url.match(/\.(mp4|webm|ogg)(\?|$)/i)) return {
        type: "video",
        src: url,
        thumb: null
    };
...
function compareProductsCatalog(a, b) {
    ...
    const ib = catalogCategoryOrderIndex(cb);
    if (ia !== ib) return ia - ib;
    const catCmp = ca.localeCompare(cb, "es", {
        sensitivity: "base",
        numeric: true
    });
    ...
    const subCmp = sa.localeCompare(sb, "es", {
        sensitivity: "base",
        numeric: true
    });
...
function getProductSearchText(p) {
    return [ p.name, p.cat, p.subcat, p.desc, (p.tags || []).join(" "), (p.apps || []).join(" ") ].join(" ").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
}
...
function fillCategorySelect() {
    ...
    if (sel) {
        sel.innerHTML = html;
        if (!cur) sel.value = ""; else if (cats.includes(cur)) sel.value = cur; else sel.value = "";
    }
    if (mob) {
        mob.innerHTML = html;
    ...
    if (!curSub || !validSubs.some(s => normFilterStr(s) === curSub)) su.value = ""; else {
...
function setSidebarOpen(open, opts = {}) {
    ...
    if (mobile) {
        ...
        if (open) acquirePageScrollLock(); else releasePageScrollLock(opts);
    } else {
...
const _SOCIAL_ICONS = {
    ...
    tiktok: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" aria-hidden="true" class="social-svg-tiktok"><path fill="#fff" d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/></svg>'
};
...
function buildSocialLinksHtml() {
    const rows = [ {
        ...
        label: "Facebook",
        cls: "social-link--fb",
        icon: _SOCIAL_ICONS.facebook
    }, {
        href: SOCIAL_INSTAGRAM,
        label: "Instagram",
        cls: "social-link--ig",
        icon: _SOCIAL_ICONS.instagram
    }, {
        href: SOCIAL_TIKTOK,
        label: "TikTok",
        cls: "social-link--tt",
        icon: _SOCIAL_ICONS.tiktok
    } ];
    return rows.map(({href: href, label: label, cls: cls, icon: icon}) => `<a class="social-link ${cls}" href="${escapeAttr(href)}" target="_blank" rel="noopener noreferrer" aria-label="P.Acoustic en ${escapeAttr(label)}">${icon}</a>`).join("");
}

function applySocialLinks() {
    const html = buildSocialLinksHtml();
    [ "socialLinksNav" ].forEach(id => {
...
function setupVoiceCatalogSearch() {
    ...
    const secure = window.isSecureContext || /^localhost$|^127\.0\.0\.1$/i.test(window.location.hostname || "");
    if (!SR || !se || !secure) {
        btnDesk?.remove();
        btnMob?.remove();
        return;
...
function setupCatalogFilters() {
    ...
    if (mob_s) {
        mob_s.addEventListener("input", () => {
            if (se) {
                se.value = mob_s.value;
                PAGINATION_CONFIG.currentPage = 1;
                renderProducts();
            }
        });
...
function openModal(id) {
    ...
    function trap(e) {
        ...
        if (lbOpen && (e.key === "ArrowRight" || e.key === "ArrowLeft")) {
            e.preventDefault();
            if (e.key === "ArrowRight") lbNav(1); else lbNav(-1);
            return;
        }
        if (e.key === "ArrowRight") {
            if (typeof modalZoomCleanup === "function") modalZoomCleanup();
...
function cerrarModalBtn() {
    ...
    if (openModal._trap) {
        document.removeEventListener("keydown", openModal._trap);
        openModal._trap = null;
    }
    if (openModal._prev) {
        openModal._prev.focus();
        openModal._prev = null;
    }
    const modalOv = document.getElementById("modalOverlay");
...
document.addEventListener("keydown", e => {
    ...
    if (pdfO && pdfO.style.display === "flex") cerrarPDF(); else cerrarModalBtn();
});
    ...
    if (!lb) {
        ...
        wrap.addEventListener("touchstart", e => {
            ...
            if (e.touches.length === 1) {
                LBState.lastX = e.touches[0].clientX;
                LBState.lastY = e.touches[0].clientY;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (sidebarCatUl) {
        sidebarCatUl.addEventListener("click", e => {
            ...
            if (btn.classList.contains("sidebar-cat--parent") && btn.getAttribute("data-has-subs") === "1" && tapFlyout) {
                const fly = document.getElementById("sidebarFlyout");
                const openSame = fly?.classList.contains("is-open") && normFilterStr(fly?.dataset.openCat || "") === normFilterStr(cat);
                if (openSame) {
                    hideCategoryFlyout();
                    return;
                }
    ...
    window.addEventListener("load", () => scheduleBannerRelayout(), {
        once: true
    });
