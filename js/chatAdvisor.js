(function() {
    ...
    function suggestForScenario(key) {
        ...
        const catGuess = key === "graves" ? "Woofer" : key === "drivers" ? "Drivers" : "Cabinas";
    ...
    function welcomeChips() {
        return [ {
            id: "eventos",
            label: "Conciertos / eventos grandes"
        }, {
            id: "iglesia",
            label: "Iglesia, teatro o auditorio"
        }, {
            id: "dj",
            label: "DJ, club o móvil"
        }, {
            id: "instalacion",
            label: "Instalación fija"
        }, {
            id: "graves",
            label: "Sub graves y woofers"
        }, {
            id: "drivers",
            label: "Drivers / agudos"
        }, {
            id: "ver_cats",
            label: "Elegir por categoría"
        } ];
    }
    function categoryChips() {
        ...
        return [ ...set ].sort((a, b) => a.localeCompare(b, "es")).map(c => ({
            id: "cat:" + c,
            label: c
        }));
    }
    function onMessagesClick(e) {
        ...
        if (!id) return;
        if (id === "wa") {
            window.open(WA + encodeURIComponent("Hola P.Acoustic, quiero asesoría para elegir el mejor equipo según mi proyecto (usé el asesor en la web)."), "_blank", "noopener,noreferrer");
            return;
        }
        ...
        if (id.startsWith("cat:")) {
            ...
            appendMsg(`<p>He filtrado por <strong>${esc(cat)}</strong>. Aquí tienes modelos para revisar; puedes abrir la ficha o seguir afinando por WhatsApp.</p>`, false);
        ...
        if (picks.length) renderProductCards(picks); else appendMsg("<p>No encontré coincidencias fuertes; revisa el catálogo completo o escríbenos.</p>", false);
        renderChips([ {
            id: "restart",
            label: "Nueva consulta"
        }, {
    ...
    function showWelcome() {
        appendMsg("<p>¡Hola! Soy el <strong>asesor de producto</strong> de P.Acoustic. Con un par de clics te propongo modelos según tu escenario (eventos, instalación, graves…).</p><p>¿Qué se acerca más a lo que buscas?</p>", false);
    ...
    function buildUI() {
        ...
        function runAdvisorTextSearch() {
            ...
            inp.value = "";
            const nq = norm(q);
            const hits = getCatalog().filter(p => norm([ p.name, p.cat, p.subcat, p.desc ].join(" ")).includes(nq)).slice(0, 6);
            if (hits.length) {
                appendMsg("<p>Modelos que coinciden con tu búsqueda:</p>", false);
                renderProductCards(hits);
            } else {
                appendMsg("<p>No hallé coincidencias exactas. Prueba otra palabra (categoría, potencia, pulgadas) o escribe por <strong>WhatsApp</strong>.</p>", false);
            }
            renderChips([ {
                id: "restart",
                label: "Nueva consulta"
        ...
        const voiceSecure = window.isSecureContext || /^localhost$|^127\.0\.0\.1$/i.test(window.location.hostname || "");
    ...
    document.addEventListener("pacoustic:catalog-ready", init, {
        once: true
    });
